...
**Smart components** sometimes called contrainer are the one that **handles state** changes.
...
#### camelCase in _declaration_

#### kebab-case in _templates_
...
```js
<script setup>const emit = defineEmits(['close-window']) function buttonClick() {emit('close-window')}</script>
```
...
#### PascalCase for _name_

#### camelCase for _members_
...
Prefer not to use either for explicit unavailability Prefer not to use either for explicit unavailability
